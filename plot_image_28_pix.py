def plot_image(i, predictions_array, true_label, img):
    ...
    predictions_array, true_label, img = predictions_array, true_label[i], img[i] # Con i se extrae fila por fila de datos
    plt.grid(False)
    plt.xticks([])
    plt.yticks([])
    ...
    predicted_label = np.argmax(predictions_array) # Se extrae el indice donde la probabilidad es mayor, este coincide con la clase
    ...
    plt.xlabel("{} {:2.0f}% ({})".format(class_names[predicted_label], # Extrae el nombre de la clase de acuerdo a lo predicho
                                100*np.max(predictions_array), # Extrae la probabilidad mayor y la convierte en porcentaje
...
def plot_value_array(i, predictions_array, true_label):
    ...
    plt.yticks([])
    thisplot = plt.bar(range(10), predictions_array, color="#777777") # Se establece el graficado de las probabilidades de la predicción para la imágen
    plt.ylim([0, 1]) # al no ser mayor a 1 se delimita el eje y en 1 como máximo (1 == 100% de coincidencia con la clase)
    predicted_label = np.argmax(predictions_array) # Se extrae la clase con mayor probabilidad (numero entero)
